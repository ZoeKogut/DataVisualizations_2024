...
class ApplicationWindow(QtWidgets.QMainWindow):
    ...
    #Update Annotation for Left graph
    def update_annot(self, ind):
        #print("ind: ", ind["ind"][0])
        pos = self.plots.get_offsets()[ind["ind"][0]]
        self.annot.xy = pos

        #Get each text beforehand and then join it all together in one big text string
        ind_index = ind["ind"][0]
        text_name = df.iloc[ind_index, 0]
        text_region = df.iloc[ind_index, 1]
        text_CO2 = df.iloc[ind_index, 2]
        text_GDP = df.iloc[ind_index, 3]
        text_airports = df.iloc[ind_index, 4]
        text_alcohol = df.iloc[ind_index, 5]
        text_area = df.iloc[ind_index, 6]
        text_birthrate = df.iloc[ind_index, 7]
        text_broadband = df.iloc[ind_index, 8]
        text_str = "Name: {}\n Region: {}\n CO2: {}\n GDP_per_capita: {}\n airports: {}\n alcohol: {}\n area: {}\n birthrate: {}\n breadband: {}"
        text = text_str.format(text_name, text_region, text_CO2, text_GDP, text_airports, 
                               text_alcohol, text_area, text_birthrate,text_broadband)
    ...
    #Update Annotation for Right Graph
    def update_annot2(self, ind):
        #print("ind: ", ind["ind"][0])
        pos = self.plots2.get_offsets()[ind["ind"][0]]
        self.annot2.xy = pos
    ...
    #Hover func for Left Graph
    def hover(self, event):
        vis = self.annot.get_visible()
        if event.inaxes == self.ax:
            cont, ind = self.plots.contains(event)
            if cont:
                self.update_annot2(ind)
                self.update_annot(ind)
                self.annot2.set_visible(True)
                self.annot.set_visible(True)
                #print("mlp: ", self.mpl_canvas.figure.canvas)
                self.mpl_canvas2.figure.canvas.draw_idle()
    ...
    #Hover func for Right Graph
    def hover2(self, event):
        vis = self.annot2.get_visible()
        if event.inaxes == self.ax2:
            cont, ind = self.plots2.contains(event)
            if cont:
                self.update_annot2(ind)
                self.update_annot(ind)
                self.annot2.set_visible(True)
    ...
    def create_plotR(self, d, x, y, colorL, size, sizeC): #This updates the graph.
        ...
        self.abrush = Brush(app=self, df=df, colors=df[color], plot=self.ax, plot2=self.ax2, cb=self.update_CSelect, cb2=self.update_CSelectR)
...
if __name__ == "__main__":
    ...
    app.mpl_canvas2.figure.canvas.mpl_connect("motion_notify_event", app.hover2)
