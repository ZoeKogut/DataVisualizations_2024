...
airlineNames = ['None', 'AA', 'AB', 'AC', 'AF', 'AZ', 'CA', 'CZ', 'DL', 'FL', 'FR', 'IB', 'KL', 'LH', 'MU', 'U2', 'UA', 'US', 'WN', 'ZH']
...
class ApplicationWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.g = geodesic.Geodesic(a=6378000, f=0.335) # You want geodesics drawn on a slightly flattened (f=0.335) spheroid of equatorial radius 6,378 km (the Earth)

        self.NoF = 1 #Starting Min NoF for edge slider
        self.imp = 1 #Starting Min Importance for airport slider
        self.ApN = 'None' #Starting filter for the airport names
        ...
        self.dropdownApNames = QComboBox(self.main_widget)
        for name in airlineNames: 
            self.dropdownApNames.addItem(name)


        layout.addWidget(NavigationToolbar(self.mpl_canvas, self),     0, 0, 1, 6)
        layout.addWidget(self.mpl_canvas,                              1, 0, 4, 5)
        layout.addWidget(self.labelNode,                               5, 0, 1, 1)
        layout.addWidget(self.sliderNode,                              5, 1, 1, 3)
        layout.addWidget(self.labelEdge,                               6, 0, 1, 1)
        layout.addWidget(self.sliderEdge,                              6, 1, 1, 3)
        layout.addWidget(QLabel('Airline Filter'),                     5, 4, 1, 1) # Names the dropdown menu
        layout.addWidget(self.dropdownApNames,                         6, 4, 1, 1) # Adds the dropdown menu
        ...
        lines = []
        segmentColor = []
        #Obtain the line segments for each geodesinc curve, append the sections of the lines to the array to feed into the LineCollection
        for j in range(0, len(self.flight_sorted_subset)): #Range of the flights 200 heaviest flights
            segmentColor.append(0)
            ap1x, ap1y = ap_names[self.flight_sorted_subset.at[j, 'origin']] #Coords for the start airport
            ap2x, ap2y = ap_names[self.flight_sorted_subset.at[j, 'destination']] #cords for the end airport

            ap1x = float(ap1x) #Change the values into float just to make sure nothing errors
            ap1y = float(ap1y)
            ap2x = float(ap2x)
            ap2y = float(ap2y)

            gline = self.g.InverseLine(ap1y, ap1x, ap2y, ap2x) # construct a geodesic between your two points
            distance = gline.s13 # For this library: point 1 is your start, point 2 is your direction, point 3 is your end point. s13 is the distance between start and end point 
            steps = np.linspace(0, distance, 50, endpoint=True) # take intermediate steps along the geodesic
            geodesic_points = [ gline.Position(x) for x in steps ] # query the corresponding latitude and longitude positions.

            #Save the first points so the array will work correctly (ie, saving the 2nd point to be used as the first point in the next loop)
            lat1 = geodesic_points[0]["lat1"]
            lon1 = geodesic_points[0]["lon1"]
            for i in geodesic_points:
                lat2 = i["lat2"]
                lon2 = i["lon2"]
                if (lat1 > 77):
                    if (lon1 <= 170 and lon2 >= -170) and (lon2 <= 170 and lon1 >= -170): #Check if it goes over the edge of the map
                        lines.append([(lon1,lat1), (lon2,lat2)])
                        segmentColor[j] += 1
                else:
                    if (lon1 <= 178 and lon2 >= -178) and (lon2 <= 178 and lon1 >= -178): #Check if it goes over the edge of the map
                        lines.append([(lon1,lat1), (lon2,lat2)])
                        segmentColor[j] += 1
                lat1 = lat2
                lon1 = lon2
        ...
        temp_norm_width = []
        for i in range(len(norm_width)): #Dup the color 50 times so every segment of the line is the same color
            for j in range(segmentColor[i]):
                temp_norm_width.append(norm_width[i])
        norm_width = np.array(temp_norm_width)
        ...
        self.dropdownApNames.activated.connect(self.update_ApFilter)
    ...
    def redraw_flights(self, NoF):
        ...
        for i in range(0, len(self.flight_sorted_subset)):
            if self.ApN == 'None' or (self.ApN in self.flight_sorted_subset.at[i, 'airlines']):
                if self.airports_sorted_subset["IATA"].isin([self.flight_sorted_subset.at[i, 'origin']]).any().any() and self.airports_sorted_subset["IATA"].isin([self.flight_sorted_subset.at[i, 'destination']]).any().any():
                    ...
                    for j in geodesic_points:
                        ...
                        lat1 = lat2
                        lon1 = lon2
                    segmentColor.append(temp)
                else: 
                    temp_flight_ss.append(0)
            else: 
                temp_flight_ss.append(0)
        
        self.flight_sorted_subset = self.flight_sorted_subset[np.array(temp_flight_ss,dtype=bool)] #Map the 1/0 to get which flights are shown
        self.flight_sorted_subset = self.flight_sorted_subset.reset_index(drop=True)
        norm_width = 1 + ((self.flight_sorted_subset['number_of_flights'] - self.minNoF)/(self.maxNoF - self.minNoF)) * (self.maxSize - 1)
        temp_norm_width = []
        print(norm_width)
        for i in range(len(norm_width)): #Dup the color 50 times so every segment of the line is the same color
            for j in range(segmentColor[i]):
                temp_norm_width.append(norm_width.at[i])
        norm_width = np.array(temp_norm_width)
        cmapBlues = mpl.colormaps["Blues"]
        self.lineColors = cmapBlues(norm_width/(10))
        self.air_plot_flights = LineCollection(lines, color=self.lineColors, linewidth=norm_width, zorder=5)
        ...
        print("End of redraw_flights")
    
    #Updates filter and redraws the edges based on them
    def update_ApFilter(self, ApF):
        self.ApN = airlineNames[ApF]
        self.redraw_flights(self.NoF)
    ...
    def update_annot_airports(self, ind, diff, pos):
            ...
            if diff == 1: #Airport?
                ...
                apt1 = self.flight_sorted_subset.iloc[ind_indexF, 0]
                apt2 = self.flight_sorted_subset.iloc[ind_indexF, 1]

                text_airport1 = ap_full_name[apt1]
                text_airport2 = ap_full_name[apt2]
                text_flights = self.flight_sorted_subset.iloc[ind_indexF, 2]
                text_airlines = ", ".join(self.flight_sorted_subset.iloc[ind_indexF, 4])
                text_str = "Airport1: {}\n Airport2: {}\n Flights: {}\n Airlines: {}"
